...
- **Leaderboard**: `/leaderboard`, ranked by average test score (min 3 tests), only `leaderboard_opt_in` users listed; join/leave via inline buttons. Admins are excluded everywhere. Every full test-results screen also shows the user's standing (see the word-memo feature list) — `RANK_MIN_TESTS` in `bot/quiz_engine.py` must stay in sync with the query's `min_tests`.
